...
> behind them is not: 47 units are written to full procedural depth and 40 are
...
establishes. A tag is carried by the topic unless a test declares one of its
own — where a subject spans a contrast a single tag cannot separate, the tests
that differ say so themselves, and the rest of the subject folds under them
rather than being dropped from it:

![Two topics matched by one tag: in each, the tests that declare that surface themselves are listed and say so, the rest of the subject is folded under them with a count, and every test shows what it still requires](docs/assets/test-paths.png)
...
| Test Units | 393 |
| Written to full procedural depth | **47** |
| Sketched | 40 |
| Outline only | 306 |
...
| — another technique for the same test | 125 |
...
| Tests declaring no capability | 38 |
| Capabilities used by no test, impacts excluded | 20 of 189 |
| Capabilities no test reaches at all | 1 of 189 |
| Capabilities with a charted route to an impact | 123 of 178 |
...
the blind oracle, deliberately -- and 123 of 178 capabilities have a charted
route to an impact.
