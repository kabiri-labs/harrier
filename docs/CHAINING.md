...
**Why not name the next test directly.** The derivation currently yields 779
unit-to-unit edges across 393 units, and that is with the far half of the chain
barely charted. Written down they would be 779 hand-maintained links, each
...
It is no longer the common case. 20 of 190
...
only when a gap is worked on. A third state is counted apart from both: 1 of 190
is named by no test at all, registered under `uncovered`, and is where the
catalogue has not arrived rather than where the chart stops. Of 393 tests, 316 have a potential
continuation, 14 establish an impact, 26 stop short, and 37 declare no
