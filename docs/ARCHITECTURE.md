...
`standards/wstg-index.yaml` is that walk, generated by `pentest-navgrid index` and
**committed**. Both halves are the point. Generated, so it cannot drift from the
catalogue it describes -- `pentest-navgrid index --check` fails when it has, and the
...
question with a separate answer -- `pentest-navgrid index --check` compares the bytes
...
Pentest NavGrid has not seen the target and cannot say what the executing code would
...
claims about that target from what they had ticked. Pentest NavGrid has never seen the
...
official title, the Pentest NavGrid topic or topics that claim it, and the Test Units
...
Content with no WSTG identifier appears under **Pentest NavGrid Extensions**. It is the
...
followed — what establishes this, what declares it a prerequisite, and the
shortest charted routes onward. That link is the map's whole purpose: the rest
of the artefact is navigated from the standard down, and this is the only way in
that starts from something a test established.
...
`pentest-navgrid chain` reads the derivation in `chain.py`, which is the same one the
...
knowledge/          the taxonomy and the units
  <domain>/
    PTN-INJ-01.topic.yaml
    PTN-INJ-01-UNION.unit.yaml
